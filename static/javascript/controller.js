...
export default class Controller {
    constructor(model) {
        ...
        document.body.addEventListener('click', (event) => {
            let elem = event.target.closest('.note__content');
            if(elem) this.onExpandCollapsNote(elem.closest('.note').dataset.noteId);
        });

        // Sort notes
        document.body.addEventListener('click', (event) => {
            let elem = event.target.closest('[data-notes-sort]');
            if(elem) this.onSortOptionChange(elem.dataset.notesSort);
        });

        // Show/Hide finished
        document.body.addEventListener('click', (event) => {
            let elem = event.target.closest('[data-notes-show-finished]');
    ...
    onSortOptionChange(option) {
        ...
        switch (option) {
            ...
            case "importance":
                ...
                break;
            default:
                throw "Sort option not registered!";
                break;
        }
    }
